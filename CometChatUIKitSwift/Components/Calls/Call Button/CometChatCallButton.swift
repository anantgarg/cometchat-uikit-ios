...
//
    ...
    public class CometChatCallButtons: UIStackView {
        ...
        public init(width: CGFloat, height: CGFloat) {
            ...
            spacing = CometChatSpacing.Spacing.s2
            distribution = .fill
            alignment = .fill
            disconnect()
            connect()
        ...
        open func setupStyle() {
            if let voiceCallButton {
                voiceCallButton.set(icon: style.audioCallIcon)
                if let voiceCallIconText { voiceCallButton.set(text: voiceCallIconText) }
            ...
            if let videoCallButton {
                videoCallButton.set(icon: style.videoCallIcon)
                if let videoCallIconText { videoCallButton.set(text: videoCallIconText) }
        ...
        open func buildButton(forUser: User) {
            ...
            if let voiceCallButton {
                ...
                guard let user, let uid = user.uid else { return }
                ...
                addArrangedSubview(voiceCallButton)
            }
            ...
            if let videoCallButton {
                ...
                addArrangedSubview(videoCallButton)
            }
        }
        ...
        private func setupOutgoingCallConfiguration(outgoingCall: CometChatOutgoingCall) {
            if let outgoingCallConfiguration {
    ...
    extension CometChatCallButtons: CometChatMessageEventListener {
        public func ccMessageSent(message _: CometChatSDK.BaseMessage, status _: MessageStatus) {}
    }

    extension CometChatCallButtons: CometChatCallDelegate {
        public func onIncomingCallReceived(incomingCall _: CometChatSDK.Call?, error _: CometChatSDK.CometChatException?) {}

        public func onOutgoingCallAccepted(acceptedCall _: CometChatSDK.Call?, error _: CometChatSDK.CometChatException?) {}

        public func onOutgoingCallRejected(rejectedCall _: CometChatSDK.Call?, error _: CometChatSDK.CometChatException?) {
            disabled = false
        }

        public func onIncomingCallCancelled(canceledCall _: CometChatSDK.Call?, error _: CometChatSDK.CometChatException?) {}
    ...
    extension CometChatCallButtons: CometChatCallEventListener {
        public func ccCallEnded(call _: Call) {
            disabled = false
        }

        public func ccCallRejected(call _: Call) {
            disabled = false
        }
