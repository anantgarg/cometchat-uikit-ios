...
import UIKit
    ...
    class CallingExtensionDecorator: DataSourceDecorator {
        ...
        public func getConferenceCallTemplate(additionalConfiguration: AdditionalConfiguration) -> CometChatMessageTemplate {
            CometChatMessageTemplate(category: MessageCategoryConstants.custom, type: conferenceCallTypeConstant, contentView: { [weak self] message, _, controller in
        ...
        override func getAuxiliaryHeaderMenu(user: User?, group: Group?, controller: UIViewController?, id _: [String: Any]?, additionalConfiguration: AdditionalConfiguration) -> UIStackView? {
