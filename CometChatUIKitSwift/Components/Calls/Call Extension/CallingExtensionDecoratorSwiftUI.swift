...
import SwiftUI
    ...
    class CallingExtensionDecoratorSwiftUI: DataSourceDecorator {
        ...
        override private init(dataSource: DataSource) {
        ...
        public convenience init(dataSource: DataSource, configuration: CallingConfigurationSwiftUI?) {
            ...
            if let uiKitSettings = CometChatUIKit.uiKitSettings {
                ...
                CometChatCalls(callsAppSettings: callAppSettings) { _ in } onError: { _ in }
            }
        ...
        public func getAudioCallTemplate(additionalConfiguration: AdditionalConfiguration?) -> CometChatMessageTemplate {
            CometChatMessageTemplate(category: MessageCategoryConstants.call, type: audioCallTypeConstant, contentView: { [weak self] message, _, _ in
        ...
        public func getVideoCallTemplate(additionalConfiguration: AdditionalConfiguration?) -> CometChatMessageTemplate {
            CometChatMessageTemplate(category: MessageCategoryConstants.call, type: videoCallTypeConstant, contentView: { [weak self] message, _, _ in
                guard let this = self else { return UIView() }
                guard let message = message as? Call else { return UIView() }
                return this.getCallActionBubble(call: message, additionalConfiguration: additionalConfiguration ?? AdditionalConfiguration())
            }, bubbleView: nil, headerView: nil, footerView: nil, bottomView: nil, options: nil)
        ...
        public func getConferenceCallTemplate(additionalConfiguration: AdditionalConfiguration) -> CometChatMessageTemplate {
            CometChatMessageTemplate(category: MessageCategoryConstants.custom, type: conferenceCallTypeConstant, contentView: { [weak self] message, _, _ in
        ...
        @objc private func handleCallBubbleTap(_ sender: UITapGestureRecognizer) {
            ...
            if let customCallback = callingConfiguration?.callBubbleConfiguration?.onClick {
                customCallback()
                return
            }
        ...
        private func getCallSettingsBuilder(user: User?, group: Group?, isAudioOnly: Bool) -> CometChatCallsSDK.CallSettingsBuilder? {
            if let groupCallSettingsBuilder = callingConfiguration?.groupCallSettingsBuilder {
        ...
        private func findMessageById(_: Int) -> BaseMessage? {
            nil
        }
        ...
        override func getAuxiliaryHeaderMenu(user: User?, group: Group?, controller _: UIViewController?, id _: [String: Any]?, additionalConfiguration: AdditionalConfiguration) -> UIStackView? {
            if let user, user.blockedByMe != true {
        ...
        private func setupConfigurationFor(callButtonsSwiftUI: CometChatCallButtonsSwiftUI, additionalConfiguration: AdditionalConfiguration) {
            if let outgoingCallConfiguration = callingConfiguration?.outgoingCallConfiguration {
    ...
    extension CallingExtensionDecoratorSwiftUI: CometChatCallDelegate {
        ...
        func onOutgoingCallAccepted(acceptedCall _: CometChatSDK.Call?, error _: CometChatSDK.CometChatException?) {}

        func onOutgoingCallRejected(rejectedCall _: CometChatSDK.Call?, error _: CometChatSDK.CometChatException?) {}
