...
open class CometChatConversations: CometChatListBase {
    ...
    var textFormatters: [CometChatTextFormatter] = ChatConfigurator.getDataSource().getTextFormatters()

    var viewModel: ConversationsViewModel = .init()
    ...
    private func setupViewModel() {
        ...
        viewModel.newMessageReceived = { [weak self] _ in
            guard let this = self else { return }
            DispatchQueue.main.async {
...
extension CometChatConversations {
    ...
    override open func tableView(_: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        if let customOptions = options?(conversation) {
            ...
            if !hideDeleteConversationOption {
                let delete = UIContextualAction(style: .destructive, title: ConversationConstants.delete) { [weak self] _, _, _ in
                    guard let this = self else { return }
    ...
    private func delete(conversation: Conversation) {
        ...
        let actionSheetController = UIAlertController(title: nil, message: ConversationConstants.deleteConversationMessage, preferredStyle: .actionSheet)

        // create an action
        let firstAction = UIAlertAction(title: ConversationConstants.delete, style: .destructive) { _ in
