...
class ConversationsViewModel: ConversationsViewModelProtocol {
    ...
    var refereshConversationRequest: ConversationRequest?
    ...
    var unreadCount: [Int] = []
    var updateTypingIndicator: ((_ row: Int, _ TypingIndicator: TypingIndicator, _ typingStatus: Bool) -> Void)?
...
extension ConversationsViewModel {
    ...
    func add(conversation: Conversation) -> Self {
        if !conversations.contains(obj: conversation) {
            ...
            insertAtIndex?(IndexPath(row: conversations.count, section: 0))
        }
        return self
    }
    ...
    func insert(conversation: Conversation, at: Int = 0) {
        ...
        insertAtIndex?(IndexPath(row: at, section: 0))
    }
    ...
    func update(conversation: Conversation) {
        ...
        if let currentRow = conversations.firstIndex(where: {
            ...
            reloadAtIndex?(IndexPath(row: currentRow, section: 0))
        }
    }
