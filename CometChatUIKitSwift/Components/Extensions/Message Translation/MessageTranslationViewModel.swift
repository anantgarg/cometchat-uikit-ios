...
public class MessageTranslationViewModel: DataSourceDecorator {
    ...
    override public func getTextMessageBubble(messageText: String?, message: TextMessage?, controller: UIViewController?, alignment: MessageBubbleAlignment, style: TextBubbleStyle?, additionalConfiguration: AdditionalConfiguration?) -> UIView? {
        if let translatedMessage = message?.metaData?["translated-message"] as? String, !translatedMessage.isEmpty {
            buildTextMessageContentView(messageText: translatedMessage, message: message, controller: controller, alignment: alignment, style: style, additionalConfiguration: additionalConfiguration)
        } else {
            super.getTextMessageBubble(messageText: messageText, message: message, controller: controller, alignment: alignment, style: style, additionalConfiguration: additionalConfiguration)
        }
    }

    override public func getTextMessageOptions(loggedInUser: User, messageObject: BaseMessage, controller: UIViewController?, group: Group?, additionalConfiguration: AdditionalConfiguration) -> [CometChatMessageOption]? {
    ...
    func buildTextMessageContentView(messageText _: String?, message: TextMessage?, controller: UIViewController?, alignment: MessageBubbleAlignment, style _: TextBubbleStyle?, additionalConfiguration: AdditionalConfiguration?) -> UIView {
    ...
    private func textMessageOption(loggedInUser _: User, messageObject: BaseMessage, controller: UIViewController?, group _: Group?) -> CometChatMessageOption {
        CometChatMessageOption(id: ExtensionConstants.messageTranslation, title: "TRANSLATE_MESSAGE".localize(), icon: AssetConstants.translate) { _ in
...
extension MessageTranslationViewModel {
    ...
    private func translateMessage(messageObject: BaseMessage, controller _: UIViewController?) {
        ...
        if let message = messageObject as? TextMessage {
            ...
            CometChat.callExtension(slug: ExtensionConstants.messageTranslation, type: .post, endPoint: "v2/translate", body: ["msgId": message.id, "languages": [systemLanguage], "text": spannedStringForMention] as [String: Any], onSuccess: { response in
                DispatchQueue.main.async {
                    if let response, let originalLanguage = response["language_original"] as? String {
