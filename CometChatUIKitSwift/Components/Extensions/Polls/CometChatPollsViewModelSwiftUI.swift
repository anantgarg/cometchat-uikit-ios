...
public class CometChatPollsViewModelSwiftUI: ObservableObject, DataSource {
    ...
    public func getLastConversationMessage(conversation: Conversation, additionalConfiguration: AdditionalConfiguration?) -> NSAttributedString? {
        if let customMessage = conversation.lastMessage as? CustomMessage, let additionalConfiguration = additionalConfiguration {
    ...
    public func getAttachmentOption(controller: UIViewController?, user: User?, group: Group?) -> CometChatMessageComposerAction? {
        return CometChatMessageComposerAction(id: ExtensionConstants.polls, text: "CUSTOM_MESSAGE_POLL".localize(), startIcon: UIImage(named: "polls.png", in: CometChatUIKit.bundle, compatibleWith: nil) ?? UIImage(), endIcon: nil, startIconTint: nil, endIconTint: nil, textColor: nil, textFont: nil)
    ...
    private func addImageToText(text: String, image: String, additionalConfiguration: AdditionalConfiguration) -> NSAttributedString? {
