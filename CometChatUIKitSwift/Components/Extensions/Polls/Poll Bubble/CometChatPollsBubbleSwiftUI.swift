...
public struct CometChatPollsBubbleSwiftUI: View {
    ...
    @State private var pollsData: PollsData = .init()
    ...
    @discardableResult
    public func set(optionCheckIcon _: UIImage?) -> Self {
        var view = self
    ...
    @discardableResult
    public func set(optionUncheckIcon _: UIImage?) -> Self {
        var view = self
    ...
    private func onSelected(pollOptions: PollOptions) {
        ...
        CometChat.callExtension(slug: "polls", type: .post, endPoint: "v2/vote", body: body as [String: Any]) { _ in
            print("Success")
        } onError: { _ in
            print("Error")
        }
