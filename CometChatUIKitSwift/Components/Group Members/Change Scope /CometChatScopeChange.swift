...
open class CometChatScopeChange: UIViewController {
    ...
    @objc open func onSaveButtonClicked() {
        if let selectedOptions, let guid = group?.guid, let uid = groupMember?.uid {
            ...
            CometChat.updateGroupMemberScope(UID: uid, GUID: guid, scope: options[selectedOptions].1, onSuccess: { [weak self] _ in
