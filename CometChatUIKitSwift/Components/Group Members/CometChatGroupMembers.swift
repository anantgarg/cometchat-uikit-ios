...
open class CometChatGroupMembers: CometChatListBase {
    ...
    public var viewModel: GroupMembersViewModel = .init()
    ...
    public var onSelectedItemProceed: ((_ groupMembers: [GroupMember]) -> Void)?
    ...
    open func configureMenu(groupMember: GroupMember) -> [UIContextualAction] {
        ...
        if GroupMembersUtils.allowKickBanUnbanMember(group: viewModel.group, groupMember: groupMember) {
            ...
            let banAction = UIContextualAction(
                style: .normal,
                title: "Ban",
                handler: { [weak self, weak groupMember] _, _, completionHandler in
                    if let groupMember {
                        completionHandler(false)
    ...
    open func onRemoveMemberSelected(for groupMember: GroupMember) {
        // create an actionSheet
        let actionSheetController = UIAlertController(title: "Kick \(groupMember.name ?? "")", message: "Are you sure you want to kick \(groupMember.name ?? "") from this group ?", preferredStyle: .actionSheet)

        // create an action
        let firstAction = UIAlertAction(title: "Yes", style: .destructive) { _ in
            DispatchQueue.main.async { [weak self] in
                guard let strongSelf = self else { return }
    ...
    open func onScopeChangeSelected(for _: GroupMember, onCancelled _: @escaping (() -> Void), onCompleted _: @escaping (() -> Void)) {}

    public func onGroupMemberBanned(action _: ActionMessage, bannedUser: User, bannedBy _: User, bannedFrom _: Group) {
        DispatchQueue.main.async {
...
extension CometChatGroupMembers {
    ...
    override open func tableView(_: UITableView, numberOfRowsInSection _: Int) -> Int {
        viewModel.isSearching ? viewModel.filteredGroupMembers.count : viewModel.groupMembers.count
    }
        ...
        if let onItemClick {
            ...
            if selectionMode == .none {
                ...
                if !viewModel.selectedGroupMembers.contains(groupMember) {
                    viewModel.selectedGroupMembers.append(groupMember)
                }
            }
        }
    }
...
public extension UIImage {
    func add(text: String, imageTint: UIColor) -> UIImage? {
        let label = UILabel()
        label.text = text
