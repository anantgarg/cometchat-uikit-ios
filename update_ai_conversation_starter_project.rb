...
swiftui_file_path = 'Components/AI/AI Conversation Starter/CometChatAIConversationStarterSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatAIConversationStarterSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for AI Conversation Starter
  ai_conversation_starter_group = nil
  ...
  # Then try to find the AI Conversation Starter group
  if ai_group
    ai_group.groups.each do |group|
      if group.name == 'AI Conversation Starter'
        ai_conversation_starter_group = group
        break
      end
    end
  end

  if ai_conversation_starter_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = ai_conversation_starter_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "AI Conversation Starter group not found"
  end
end
...
uikit_file_path = 'Components/AI/AI Conversation Starter/CometChatAIConversationStarter.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatAIConversationStarter.swift') }
end
