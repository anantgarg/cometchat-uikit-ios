...
swiftui_file_path = 'Components/AI/AI Summarizer/CometChatAIConversationSummarySwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatAIConversationSummarySwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for AI Summarizer
  ai_summarizer_group = nil
  ...
  # Then try to find the AI Summarizer group
  if ai_group
    ai_group.groups.each do |group|
      if group.name == 'AI Summarizer'
        ai_summarizer_group = group
        break
      end
    end
  end

  if ai_summarizer_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = ai_summarizer_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "AI Summarizer group not found"
  end
end
...
uikit_file_path = 'Components/AI/AI Summarizer/CometChatAIConversationSummary.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatAIConversationSummary.swift') }
end
