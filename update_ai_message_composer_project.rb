...
swiftui_file_path = 'Components/AI/AI Bot/AI Message Compoer/AIMessageComposerSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('AIMessageComposerSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for AI Message Compoer
  ai_message_composer_group = nil
  ...
  # Then try to find the AI Bot group
  ai_bot_group = nil
  if ai_group
    ai_group.groups.each do |group|
      if group.name == 'AI Bot'
        ai_bot_group = group
        break
      end
    end
  ...
  # Then try to find the AI Message Compoer group
  if ai_bot_group
    ai_bot_group.groups.each do |group|
      if group.name == 'AI Message Compoer'
        ai_message_composer_group = group
        break
      end
    end
  end

  if ai_message_composer_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = ai_message_composer_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "AI Message Compoer group not found"
  end
end
...
uikit_file_path = 'Components/AI/AI Bot/AI Message Compoer/AIMessageComposer.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('AIMessageComposer.swift') }
end
