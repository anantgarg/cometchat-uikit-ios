...
swiftui_file_path = 'Components/Calls/Call Bubble/CometChatCallBubbleSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatCallBubbleSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Call Bubble
  call_bubble_group = nil
  project.groups.each do |group|
    if group.name == 'Call Bubble'
      call_bubble_group = group
      break
    end
  end

  if call_bubble_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = call_bubble_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Call Bubble group not found"
  end
end
...
uikit_file_path = 'Components/Calls/Call Bubble/CometChatCallBubble.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatCallBubble.swift') }
end
