...
swiftui_file_path = 'Components/Calls/Call Log/Call Logs/CallLogsViewModelSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CallLogsViewModelSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Call Logs
  call_logs_group = nil
  project.groups.each do |group|
    if group.name == 'Call Logs'
      call_logs_group = group
      break
    end
  end
  ...
  if call_logs_group
    ...
    if File.exist?(file_path)
      swiftui_file_ref = call_logs_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Call Logs group not found"
  end
end
...
uikit_file_path = 'Components/Calls/Call Log/Call Logs/CallLogsViewModel.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CallLogsViewModel.swift') }
end
