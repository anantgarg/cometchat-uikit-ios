...
swiftui_file_path = 'Components/Calls/Call Extension/CallingDefaultBuilderSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CallingDefaultBuilderSwiftUI.swift') }
end
...
uikit_file_path = 'Components/Calls/Call Extension/CallingDefaultBuilder.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CallingDefaultBuilder.swift') }
end
