...
swiftui_file_path = 'Components/Calls/Call Extension/CallingExtensionDecoratorSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CallingExtensionDecoratorSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Call Extension
  call_extension_group = nil
  project.groups.each do |group|
    if group.name == 'Call Extension'
      call_extension_group = group
      break
    end
  end

  if call_extension_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = call_extension_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Call Extension group not found"
  end
end
...
uikit_file_path = 'Components/Calls/Call Extension/CallingExtensionDecorator.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CallingExtensionDecorator.swift') }
end
