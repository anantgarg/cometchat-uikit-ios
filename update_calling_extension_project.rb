...
swiftui_file_path = 'Components/Calls/Call Extension/CallingExtensionSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CallingExtensionSwiftUI.swift') }
end
...
uikit_file_path = 'Components/Calls/Call Extension/CallingExtension.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CallingExtension.swift') }
end
