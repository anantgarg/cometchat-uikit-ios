...
swiftui_file_path = 'Components/Extensions/Collaborative Document/CollaborativeDocumentViewModelSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CollaborativeDocumentViewModelSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Collaborative Document
  collaborative_document_group = nil
  ...
  # Then try to find the Collaborative Document group
  if extensions_group
    extensions_group.groups.each do |group|
      if group.name == 'Collaborative Document'
        collaborative_document_group = group
        break
      end
    end
  end

  if collaborative_document_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = collaborative_document_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Collaborative Document group not found"
  end
end
...
uikit_file_path = 'Components/Extensions/Collaborative Document/CollaborativeDocumentViewModel.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CollaborativeDocumentViewModel.swift') }
end
