...
swiftui_file_path = 'Components/Extensions/Collaborative Whiteboard/CollaborativeWhiteboardExtensionSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CollaborativeWhiteboardExtensionSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Collaborative Whiteboard
  whiteboard_group = nil
  ...
  # Then try to find the Collaborative Whiteboard group
  if extensions_group
    extensions_group.groups.each do |group|
      if group.name == 'Collaborative Whiteboard'
        whiteboard_group = group
        break
      end
    end
  end
  ...
  if whiteboard_group
    ...
    if File.exist?(file_path)
      swiftui_file_ref = whiteboard_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Collaborative Whiteboard group not found"
  end
end
...
uikit_file_path = 'Components/Extensions/Collaborative Whiteboard/CollaborativeWhiteboardExtension.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CollaborativeWhiteboardExtension.swift') }
end
