...
swiftui_file_path = 'Components/Extensions/Polls/CometChatCreatePollSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatCreatePollSwiftUI.swift') }
end
...
uikit_file_path = 'Components/Extensions/Polls/CometChatCreatePoll.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatCreatePoll.swift') }
end
