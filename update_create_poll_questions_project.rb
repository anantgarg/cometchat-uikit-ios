...
swiftui_file_path = 'Components/Extensions/Polls/Create Poll Questions/CometChatCreatePollQuestionsSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatCreatePollQuestionsSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Create Poll Questions
  create_poll_questions_group = nil
  ...
  # Then try to find the Polls group
  polls_group = nil
  if extensions_group
  ...
  # Then try to find the Create Poll Questions group
  if polls_group
    polls_group.groups.each do |group|
      if group.name == 'Create Poll Questions'
        create_poll_questions_group = group
        break
      end
    end
  end

  if create_poll_questions_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = create_poll_questions_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Create Poll Questions group not found"
  end
end
...
uikit_file_path = 'Components/Extensions/Polls/Create Poll Questions/CometChatCreatePollQuestions.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatCreatePollQuestions.swift') }
end
