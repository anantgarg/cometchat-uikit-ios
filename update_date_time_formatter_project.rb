...
swiftui_file_path = 'Components/Extensions/Date Time Formatter/CometChatDateTimeFormatterSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatDateTimeFormatterSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Date Time Formatter
  date_time_formatter_group = nil
  ...
  # Then try to find the Date Time Formatter group
  if extensions_group
    extensions_group.groups.each do |group|
      if group.name == 'Date Time Formatter'
        date_time_formatter_group = group
        break
      end
    end
  end

  if date_time_formatter_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = date_time_formatter_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Date Time Formatter group not found"
  end
end
...
uikit_file_path = 'Components/Extensions/Date Time Formatter/CometChatDateTimeFormatter.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatDateTimeFormatter.swift') }
end
