...
swiftui_file_path = 'Components/Group Members/GroupMembersViewModelSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('GroupMembersViewModelSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Group Members
  group_members_group = nil
...
uikit_file_path = 'Components/Group Members/GroupMembersViewModel.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('GroupMembersViewModel.swift') }
end
