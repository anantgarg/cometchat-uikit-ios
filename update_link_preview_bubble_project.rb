...
swiftui_file_path = 'Components/Extensions/Link Preview/LinkPreviewMessage/CometChatLinkPreviewBubbleSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatLinkPreviewBubbleSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for LinkPreviewMessage
  link_preview_message_group = nil
  ...
  # Then try to find the Link Preview group
  link_preview_group = nil
  if extensions_group
    extensions_group.groups.each do |group|
      if group.name == 'Link Preview'
        link_preview_group = group
        break
      end
    end
  end
  
  # Then try to find the LinkPreviewMessage group
  if link_preview_group
    link_preview_group.groups.each do |group|
      if group.name == 'LinkPreviewMessage'
        link_preview_message_group = group
        break
      end
    end
  end

  if link_preview_message_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = link_preview_message_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "LinkPreviewMessage group not found"
  end
end
...
uikit_file_path = 'Components/Extensions/Link Preview/LinkPreviewMessage/CometChatLinkPreviewBubble.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatLinkPreviewBubble.swift') }
end
