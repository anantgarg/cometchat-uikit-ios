...
swiftui_file_path = 'Components/Extensions/Link Preview/LinkPreviewExtensionSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('LinkPreviewExtensionSwiftUI.swift') }
end
...
uikit_file_path = 'Components/Extensions/Link Preview/LinkPreviewExtension.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('LinkPreviewExtension.swift') }
end
