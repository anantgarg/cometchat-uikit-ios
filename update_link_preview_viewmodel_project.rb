...
swiftui_file_path = 'Components/Extensions/Link Preview/LinkPreviewViewModelSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('LinkPreviewViewModelSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Link Preview
  ...
  if link_preview_group
    ...
    if File.exist?(file_path)
      swiftui_file_ref = link_preview_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Link Preview group not found"
  end
end
...
uikit_file_path = 'Components/Extensions/Link Preview/LinkPreviewViewModel.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('LinkPreviewViewModel.swift') }
end
