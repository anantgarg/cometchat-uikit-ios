...
swiftui_file_path = 'Components/Message List/MessageListViewModelSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('MessageListViewModelSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Message List
  message_list_group = nil
  project.groups.each do |group|
    if group.name == 'Message List'
      message_list_group = group
      break
    end
  end

  if message_list_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = message_list_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Message List group not found"
  end
end
...
uikit_file_path = 'Components/Message List/MessageListViewModel.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('MessageListViewModel.swift') }
end
