...
swiftui_file_path = 'Components/Extensions/Message Translation/CometChatMessageTranslationBubbleSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatMessageTranslationBubbleSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Message Translation
  message_translation_group = nil
  ...
  # Then try to find the Message Translation group
  if extensions_group
    extensions_group.groups.each do |group|
      if group.name == 'Message Translation'
        message_translation_group = group
        break
      end
    end
  end

  if message_translation_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = message_translation_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Message Translation group not found"
  end
end
...
uikit_file_path = 'Components/Extensions/Message Translation/CometChatMessageTranslationBubble.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatMessageTranslationBubble.swift') }
end
