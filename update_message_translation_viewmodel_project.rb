...
swiftui_file_path = 'Components/Extensions/Message Translation/MessageTranslationViewModelSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('MessageTranslationViewModelSwiftUI.swift') }
end
...
uikit_file_path = 'Components/Extensions/Message Translation/MessageTranslationViewModel.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('MessageTranslationViewModel.swift') }
end
