...
swiftui_file_path = 'Components/Extensions/Polls/Poll Bubble/CometChatPollsBubbleSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatPollsBubbleSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Poll Bubble
  poll_bubble_group = nil
  ...
  # Then try to find the Poll Bubble group
  if polls_group
    polls_group.groups.each do |group|
      if group.name == 'Poll Bubble'
        poll_bubble_group = group
        break
      end
    end
  end

  if poll_bubble_group
    # Add the file to the project
    file_path = "CometChatUIKitSwift/#{swiftui_file_path}"
    if File.exist?(file_path)
      swiftui_file_ref = poll_bubble_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Poll Bubble group not found"
  end
end
...
uikit_file_path = 'Components/Extensions/Polls/Poll Bubble/CometChatPollsBubble.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatPollsBubble.swift') }
end
