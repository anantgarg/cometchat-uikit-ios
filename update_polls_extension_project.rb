...
swiftui_file_path = 'Components/Extensions/Polls/CometChatPollsExtensionSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatPollsExtensionSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Polls
  polls_group = nil
...
uikit_file_path = 'Components/Extensions/Polls/CometChatPollsExtension.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatPollsExtension.swift') }
end
