...
swiftui_file_path = 'Components/Extensions/Profanity Data Masking/ProfanityDataMaskingExtensionDecoratorSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('ProfanityDataMaskingExtensionDecoratorSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Profanity Data Masking
  profanity_group = nil
  ...
  # Then try to find the Profanity Data Masking group
  if extensions_group
    extensions_group.groups.each do |group|
      if group.name == 'Profanity Data Masking'
        profanity_group = group
        break
      end
    end
  end
  ...
  if profanity_group
    ...
    if File.exist?(file_path)
      swiftui_file_ref = profanity_group.new_file(file_path)
      puts "Added SwiftUI file to project"
    else
      puts "SwiftUI file not found on disk: #{file_path}"
    end
  else
    puts "Profanity Data Masking group not found"
  end
end
...
uikit_file_path = 'Components/Extensions/Profanity Data Masking/ProfanityDataMaskingExtensionDecorator.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('ProfanityDataMaskingExtensionDecorator.swift') }
end
