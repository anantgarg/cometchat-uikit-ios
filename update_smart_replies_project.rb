...
swiftui_file_path = 'Components/Extensions/Smart Replies/CometChatSmartRepliesSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('CometChatSmartRepliesSwiftUI.swift') }
end
...
uikit_file_path = 'Components/Extensions/Smart Replies/CometChatSmartReplies.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('CometChatSmartReplies.swift') }
end
