...
swiftui_file_path = 'Components/Extensions/Sticker/StickersExtensionDecoratorSwiftUI.swift'
...
if swiftui_file_ref.nil?
  swiftui_file_ref = project.files.find { |f| f.path.end_with?('StickersExtensionDecoratorSwiftUI.swift') }
end

# If still not found, we need to add the file to the project
if swiftui_file_ref.nil?
  # Find the group for Sticker
  sticker_group = nil
  ...
  # Then try to find the Sticker group
  if extensions_group
    extensions_group.groups.each do |group|
      if group.name == 'Sticker'
        sticker_group = group
        break
      end
    end
...
uikit_file_path = 'Components/Extensions/Sticker/StickersExtensionDecorator.swift'
uikit_file_ref = project.files.find { |f| f.path == uikit_file_path }

# If not found by path, try to find by name
if uikit_file_ref.nil?
  uikit_file_ref = project.files.find { |f| f.path.end_with?('StickersExtensionDecorator.swift') }
end
